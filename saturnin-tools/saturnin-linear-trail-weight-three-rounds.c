...
#define NUMBER_OF_CANDIDATES 349
...
main(void)
{
	// We know that the number of candidate masks is NUMBER_OF_CANDIDATES, because we used the code below
	// to compute and print them. 
	uint16_t candidate_masks[NUMBER_OF_CANDIDATES];
	...
	Table_fixed *weights[NUMBER_OF_CANDIDATES];
       	for (long i = 0; i < NUMBER_OF_CANDIDATES; i++) {
	...
	#pragma omp parallel for schedule(dynamic)
		// Consider each candidate mask
		for (long i = 0; i < NUMBER_OF_CANDIDATES; i++) {
			mpz_t val;
			mpz_init(val);
			uint16_t x = candidate_masks[i];
			int box_weight_a = hamming_weight16(x);
			// The number of iterations of loop is bounded from above by 2^16, since w <= 4
			// Step over all possible masks after the second Sbox-layer,
			...
			for (uint64_t a = 0; a < (uint64_t) pow(2, 4*box_weight_a); a++) {
				...
				// Build the mask before the third Sbox-layer
				uint64_t state1 = 0;
				...
				// Build the mask after the second Sbox-layer
				state1 = permute_slice(state1);
				state1 = mix_columns_slice_transposed_inverse(state1);
				...
				for (long j = 0; j < 16; j++) {
					...
					count_output(val, (state1 >> 4*j) & 0xf, 8, j);
					table_fixed_insert_and_merge(bases[16+j], 2, val, &mpz_add);

					count_output(val, (state1 >> 4*j) & 0xf, 4, j);
					table_fixed_insert_and_merge(bases[16+j], 4, val, &mpz_add);
				}
				...
				// For a fixed mask AFTER the second Sbox-layer, consider all the masks BEFORE the second Sbox-layer, 
				// the number of which is bounded from above by 2^12 (empirically verified)
				for (uint64_t b = 0; b < (uint64_t) pow(2, 4*box_weight_a); b++) {
					bool valid = true;
					long linear_weight_middle = 0; 
					for (long j = 0, m = 0; j < 16 && m < box_weight_a; j++) {
						if ((x & (1U << j)) != 0) {
							...
							long count;
							if (j % 2 == 0) {
								count = correlation_table_even[16*row_index+column_index];
							} else {
								count = correlation_table_odd[16*row_index+column_index];
							}
							if (count == 0) {
								valid = false;
								break;
							}
							linear_weight_middle += 2*(4 - (long) log2((double) abs(count)));
							m++;
						}
					}
					...
					if (valid) {
						...
						state0 = mix_columns_slice_transposed(state0);
			...
			char filename[60];
			sprintf(filename, "data/saturnin-linear-trail-three-rounds_%04x.txt", x);
			table_fixed_print(filename, weights[i]);
		}

	for (long i = 0; i <= BOUND; i++) {
		for (long j = 0; j < NUMBER_OF_CANDIDATES; j++) {
			table_fixed_insert_and_merge(total_weight, i, weights[j]->head[i], &mpz_add);
		}
	}

	table_fixed_print("data/saturnin-linear-trail-three-rounds_total.txt", total_weight);
